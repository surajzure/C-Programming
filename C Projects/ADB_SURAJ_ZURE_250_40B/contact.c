...
BATCH ID :25026_285
Project Description:
The Address Book is a console-based application developed in C programming that helps users 
store and manage contact information efficiently.
...
user-friendly.
The application also supports displaying all saved contacts in a structured format for easy 
viewing. 
...
int create_contact(AddressBook *addressbook)
{
    ...
    switch (add_contact)
    {
        ...
        default:
            ...
            goto add_contact_menu; // GO TO THE TAGE
    }
}
...
int search_contacts(AddressBook *addressbook)
{
    ...
    while (1)
    {
        ...
        {
        case 1:
            only_search_by_name(addressbook); //FUNCTION CALL
            break;

        case 2:
            only_search_by_phone_number(addressbook);//FUNCTION CALL
            break;

        case 3:
            only_search_by_Email_ID(addressbook);//FUNCTION CALL
            break;
...
int edit_contact(AddressBook *addressbook)
{
    ...
    while (1)
    {
        ...
        case 1:
        {
            ...
            index = Search_by_name_and_Edit(addressbook); //FUNCTION CALL
        ...
        case 2:
        {
            ...
            index = Search_by_phone_number_and_Edit(addressbook); //FUNCTION CALL
            break;
        }
        case 3:
        {
            // implementation for Mail ID search
            index = Search_by_Email_ID_and_Edit(addressbook); //FUNCTION CALL
            break;
        }
        case 4:
        {
...
int delete_contact(AddressBook *addressbook)
{
    ...
    while (1)
    {
        ...
        {
        case 1:
            index = Search_by_name_and_Delete(addressbook);  //FUNCTION CALL
            break;

        case 2:
            index = Search_by_phone_number_and_Delete(addressbook); //FUNCTION CALL
            break;

        case 3:
            index = Search_by_Email_ID_and_Delete(addressbook); //FUNCTION CALL
            break;
        ...
        {
            addressbook->contact_details[i] = addressbook->contact_details[i + 1]; /*Shifting logic */
        }
...
{
    FILE *fptr; /*File poiner */
    int i;

    fptr = fopen("contact_data_base.txt", "w"); /*Open the file*/

    if (fptr == NULL) /*check file is NULL or not*/
    {
...
int load_contact(AddressBook *addressbook)
{
    ...
    fptr = fopen("contact_data_base.txt", "r"); /*OPEN THE FILE*/
    ...
    {
        fscanf(fptr, "%[^,],%[^,],%[^\n]\n",addressbook->contact_details[i].Name,addressbook->contact_details[i].Mobile_number,addressbook->contact_details[i].Mail_ID);
    }/*Selective because we have to scan up to (,)*/
...
int name_validation(char *name)
{
    ...
    if (name[0] == '\0') // if 0 th index is space 
    {
    ...
    while (name[i] != '\0')
    {
        if (isupper(name[i]) || islower(name[i]) || name[i] == ' ') // CHECKING UPEER CASE AND LOWER CASE AND SPACE 
        {
            if (isupper(name[i]) || islower(name[i]))
                count++; // count only alphabets OR CHARACTERS
        }
        else if (isdigit(name[i])) // IF DIGIT 
        {
            printf(RED"❌ Invalid Name!\n");
            printf("➡ Name should NOT contain digits. Please try again.\n"RESET);
            return 0;
        }
        else // IF ANY OTHER SPACIAL CHAR 
        {
            printf(RED"❌ Invalid Name!\n");
    ...
    if (count > 0) // IF COUNT IS SMALLER THAN 0 
    {
...
int phone_number_validation(char *phone_no, AddressBook *addressbook)
{
    ...
    /* First digit check */
    if (!(phone_no[0] >= '6' && phone_no[0] <= '9')) // CHECKING THE FIRST INDEX IT SHOULD BE BETWEEN 6 TO 9
    {
    ...
    while (phone_no[i] != '\0')
    {
        if (!isdigit(phone_no[i])) // IF NOT DIGIT THE RETURN ZERO
        {
            printf(RED"❌ Invalid Mobile Number!\n");
    ...
    for (i = 0; i < addressbook->contact_count; i++)
    {
        if (strcmp(addressbook->contact_details[i].Mobile_number, phone_no) == 0)  // COMPARING WITH EXISTING DATA
        {
...
int mail_id_validation(char *Email_ID, AddressBook *addressbook)
{
    ...
    for (i = 0; i < addressbook->contact_count; i++)
    {
        if (strcmp(addressbook->contact_details[i].Mail_ID, Email_ID) == 0) // CHECKING WITH EXISTING DATA
        {
...
void only_search_by_name(AddressBook *addressbook)
{
    ...
    for (i = 0; i < addressbook->contact_count; i++)
    {
        if (strcasestr(addressbook->contact_details[i].Name, name) != NULL) // CHECKING WITH EXISTING DATA
        {                                            /* Main string(dest)  , new string(source)*/
...
int Search_by_name_and_Edit(AddressBook *addressbook)
{
    ...
    index = edit_index[index - 1]; /*index -1 because array start index starts frome 0*/
